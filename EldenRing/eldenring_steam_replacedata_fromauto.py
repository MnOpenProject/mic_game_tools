''' 用自动备份的记录文件覆盖游戏当前记录的脚本：即选择一个已备份的记录，复制到游戏记录的目录下，让游戏运行时进行使用 '''
''' 如果还原游戏记录之前忘记先主动备份了，又想还原某个之前的记录，那么可以选择自动备份的记录文件 '''
import os,shutil,getpass,time
from config import auto_datacopy_remember_dir
...
# 为 API 提供的功能函数
def replacedata_fromauto_action_forapi(filename):
    # 获取当前系统登录用户名
    ...
    if not os.path.exists(game_data_dir):
        ...
        # return None
    # 判断是否已有备份的记录文件
    data_files = os.listdir(auto_datacopy_remember_dir)
    if len(data_files) < 1:
        print(f'{auto_datacopy_remember_dir}目录下，目前没有任何备份记录，请先备份记录')
        return None
    ...
    # 把当前选择的记录复制过去
    source_dir = f'{auto_datacopy_remember_dir}/{filename}'
    print(f'当前选择的备份记录为: \n{source_dir}')
...
def replacedata_action():
    ...
    shutil.rmtree(game_data_dir)
    # 把当前选择的记录复制过去
    source_dir = f'{auto_datacopy_remember_dir}/{data_files[select_idx]}'
    print(f'当前选择的备份记录为: \n{source_dir}')
    target_dir = game_data_dir
    shutil.copytree(source_dir,target_dir)
    return target_dir
